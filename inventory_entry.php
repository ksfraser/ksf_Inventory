...
if (isset($_GET['NewDelivery']) && is_numeric($_GET['NewDelivery'])) {
	...
	create_cart(ST_Inventory, $_GET['NewInvoice']);
	...
	create_cart(ST_Inventory, $_GET['ModifyOrderNumber']);
	...
	create_cart(ST_Inventory, $_GET['ModifyQuotationNumber']);

} elseif (isset($_GET['NewOrder'])) {

	$_SESSION['page_title'] = _($help_context = "New Sales Order Entry");
	create_cart(ST_Inventory, 0);
	...
	create_cart(ST_Inventory, $_GET['NewQuoteToSalesOrder']);
}
...
if (isset($_GET['AddedID'])) {
	...
	submenu_view(_("&View This Order"), ST_Inventory, $order_no);

	submenu_print(_("&Print This Order"), ST_Inventory, $order_no, 'prtopt');
	submenu_print(_("&Email This Order"), ST_Inventory, $order_no, null, 1);
	set_focus('prtopt');
	...
	submenu_view(_("&View This Quotation"), ST_Inventory, $order_no);

	submenu_print(_("&Print This Quotation"), ST_Inventory, $order_no, 'prtopt');
	submenu_print(_("&Email This Quotation"), ST_Inventory, $order_no, null, 1);
	set_focus('prtopt');
	...
	submenu_option(_("Select A Different &Quotation"),
		"/sales/inquiry/sales_orders_view.php?type=".ST_Inventory);
	...
	submenu_view(_("&View This Invoice"), ST_Inventory, $invoice);

	submenu_print(_("&Print Sales Invoice"), ST_Inventory, $invoice."-".ST_Inventory, 'prtopt');
	submenu_print(_("&Email Sales Invoice"), ST_Inventory, $invoice."-".ST_Inventory, null, 1);
	set_focus('prtopt');
	
	$sql = "SELECT trans_type_from, trans_no_from FROM ".TB_PREF."cust_allocations
			WHERE trans_type_to=".ST_Inventory." AND trans_no_to=".db_escape($invoice);
	...
	display_note(get_gl_view_str(ST_Inventory, $invoice, _("View the GL &Journal Entries for this Invoice")),0, 1);
	...
	submenu_option(_("Add an Attachment"), "/admin/attachments.php?filterType=".ST_Inventory."&trans_no=$invoice");
...
function copy_to_cart()
{
	...
	if ($cart->trans_type!=ST_Inventory && $cart->trans_type!=ST_Inventory) { // 2008-11-12 Joe Hunt
...
function can_process() {
	...
	if ($_SESSION['Items']->trans_type!=ST_Inventory && $_SESSION['Items']->trans_type!=ST_Inventory && !is_date_in_fiscalyear($_POST['OrderDate'])) {
	...
	if ($_SESSION['Items']->payment_terms['cash_sale'] == 0) {
		...
		if ($_SESSION['Items']->trans_type != ST_Inventory && strlen($_POST['delivery_address']) <= 1) {
		...
		if (!is_date($_POST['delivery_date'])) {
			if ($_SESSION['Items']->trans_type==ST_Inventory)
...
function  handle_cancel_order()
{
	...
	} elseif ($_SESSION['Items']->trans_type == ST_Inventory) {
	...
	} elseif ($_SESSION['Items']->trans_type == ST_Inventory)
	{
		...
		if ($_SESSION['Items']->trans_no != 0) {
			...
			if (sales_order_has_deliveries($order_no))
			{
				...
				submenu_option(_("Select Another Sales Order for Edition"), "/sales/inquiry/sales_orders_view.php?type=".ST_Inventory);
			} else {
				delete_sales_order(key($_SESSION['Items']->trans_no), $_SESSION['Items']->trans_type);
...
{ 
	...
	if (isset($_GET['NewQuoteToSalesOrder']))
	{
		...
		$doc = new Cart(ST_Inventory, $trans_no, true);
		$doc->Comments = _("Sales Quotation") . " # " . $trans_no;
		$_SESSION['Items'] = $doc;
	}	
	elseif($type != ST_Inventory && $type != ST_Inventory && $trans_no != 0) { // this is template

		$doc = new Cart(ST_Inventory, array($trans_no));
		$doc->trans_type = $type;
		$doc->trans_no = 0;
		$doc->document_date = new_doc_date();
		if ($type == ST_Inventory) {
